...
impl SlotFetcher {
	...
	pub async fn fetch_and_compare_batch(&self, batch_number: u64) -> Result<LedgerBatch, anyhow::Error> {
		let batch = self.client.get_batch_by_id(&types::IntOrHash::Integer(batch_number), None).await?.into_inner();
		let batch_by_hash = self.client.get_batch_by_id(&types::IntOrHash::Hash(batch.hash.clone()), Some(GetBatchByIdChildren::_0)).await?.into_inner();
		let mut batch_with_children = self.client.get_batch_by_id(&types::IntOrHash::Integer(batch_number), Some(GetBatchByIdChildren::_1)).await?.into_inner();
		let batch_by_hash_with_children = self.client.get_batch_by_id(&types::IntOrHash::Hash(batch.hash.clone()), Some(GetBatchByIdChildren::_1)).await?.into_inner();
